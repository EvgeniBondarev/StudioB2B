...
{
    // Queries
    public static async Task<List<PermissionDto>> GetPermissionsAsync(this TenantDbContext db, CancellationToken ct = default)
    {
    ...
    public static async Task<PermissionDto?> GetPermissionByIdAsync(this TenantDbContext db, Guid id, CancellationToken ct = default)
    {
    ...
    public static Task<List<LabelValueDto>> GetAvailablePermissionsAsync(this TenantDbContext db, CancellationToken ct = default)
    {
    ...
    public static async Task<List<PageWithDetailsDto>> GetPagesWithDetailsAsync(this TenantDbContext db, CancellationToken ct = default)
    {
    ...
    public static async Task<Dictionary<string, List<EntityOptionDto>>> GetEntityOptionsForPermissionAsync(this TenantDbContext db, CancellationToken ct = default)
    {
        ...
        result[BlockedEntityTypeEnum.MarketplaceClient.ToString()] =
            db.MarketplaceClients == null ? new List<EntityOptionDto>() :
    ...
    // Commands
    public static async Task<(bool Success, string? Error, Guid Id)> CreatePermissionAsync(this TenantDbContext db, CreatePermissionDto request, CancellationToken ct = default)
    {
    ...
    public static async Task<(bool Success, string? Error)> UpdatePermissionAsync(this TenantDbContext db, Guid id, UpdatePermissionDto request, CancellationToken ct = default)
    {
    ...
    public static async Task<(bool Success, string? Error)> DeletePermissionAsync(this TenantDbContext db, Guid id, CancellationToken ct = default)
    {
        var perm = await db.Permissions.FindAsync(new object[] { id }, ct);
        if (perm is null) return (false, "Право не найдено");
        perm.IsDeleted = true;
        await db.SaveChangesAsync(ct);
        return (true, null);
