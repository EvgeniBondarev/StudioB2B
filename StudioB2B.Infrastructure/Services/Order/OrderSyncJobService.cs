...
public class OrderSyncJobService : IOrderSyncJobService
{
    ...
    public OrderSyncJobService(ITenantDbContextCreator dbCreator, ITenantProvider tenantProvider,
                               TenantHangfireManager hangfireManager, ICurrentUserProvider currentUserProvider)
    {
    ...
    public async Task CancelJobAsync(string hangfireJobId)
    {
        ...
        if (history is not null && history.Status is SyncJobStatusEnum.Enqueued or SyncJobStatusEnum.Processing)
        {
    ...
    private void EnsureTenantResolved()
    {
        if (!_tenantProvider.IsResolved)
            throw new InvalidOperationException("Tenant is not resolved. Cannot enqueue sync job.");
    }

    /// <summary>
    /// Регистрирует recurring job в Hangfire, явно разрешая неоднозначность перегрузок AddOrUpdate.
    /// </summary>
    private static void RegisterSchedule(RecurringJobManager manager, string recurringJobId, Guid tenantId,
                                         string connectionString, Guid scheduleId, string cron)
    {
        // Используем прямой вызов через Job-объект чтобы избежать CS0121 (ambiguous overloads)
        var job = Job.FromExpression<OrderSyncJob>(j => j.ExecuteScheduledAsync(tenantId, connectionString, scheduleId));
