...
public class OrderTransactionService : IOrderTransactionService
{
    ...
    public async Task<TransactionApplyPreviewDto?> GetApplyPreviewAsync(Guid orderId, Guid transactionId, CancellationToken ct = default)
    {
        ...
        var rules = new List<TransactionApplyRulePreviewDto>();
        ...
        foreach (var rule in transaction.Rules.OrderBy(r => r.SortOrder))
        {
            ...
            rules.Add(new TransactionApplyRulePreviewDto
            {
                RuleId = rule.Id,
        ...
        var fieldRules = new List<TransactionApplyFieldRulePreviewDto>();
        foreach (var fr in transaction.FieldRules.OrderBy(r => r.SortOrder))
        {
            ...
            fieldRules.Add(new TransactionApplyFieldRulePreviewDto
            {
                RuleId = fr.Id,
        ...
        return new TransactionApplyPreviewDto
        {
    ...
    public async Task<TransactionApplyPreviewDto?> GetApplyPreviewWithUserValuesAsync(Guid orderId, Guid transactionId, IReadOnlyDictionary<Guid, decimal>? userValues, CancellationToken ct = default)
    {
    ...
    public async Task<TransactionApplyResultDto> ApplyAsync(Guid orderId, Guid transactionId, IReadOnlyDictionary<Guid, decimal>? ruleValues = null, IReadOnlyDictionary<Guid, string>? fieldRuleValues = null, CancellationToken ct = default)
    {
        var order = await _db.Orders
            .IncludeForGrid()
        ...
        if (order == null)
            return new TransactionApplyResultDto { Success = false, ErrorMessage = "Заказ не найден" };
        ...
        if (transaction == null)
            return new TransactionApplyResultDto { Success = false, ErrorMessage = "Документ не найден" };

        if (!transaction.IsEnabled)
        {
            await AddHistoryAsync(orderId, transactionId, false, "Документ отключён", 0, 0, ct);
            await _db.SaveChangesAsync(ct);
            return new TransactionApplyResultDto { Success = false, ErrorMessage = "Документ отключён" };
        }
        ...
        if (order.SystemStatusId != transaction.FromSystemStatusId)
        {
            ...
            await _db.SaveChangesAsync(ct);
            return new TransactionApplyResultDto { Success = false, ErrorMessage = msg };
        }
        ...
        if (validationErrors.Count > 0)
        {
            ...
            await AddHistoryAsync(orderId, transactionId, false, msg, 0, 0, ct);
            await _db.SaveChangesAsync(ct);
            return new TransactionApplyResultDto { Success = false, ErrorMessage = msg, ValidationErrors = validationErrors };
        }
        ...
        return new TransactionApplyResultDto
        {
            Success = true,
    ...
    private static bool ApplyFieldValue(OrderEntity order, string entityPath, string? valueStr, TransactionFieldValueTypeEnum valueType)
    {
        try
        {
            switch (entityPath)
    ...
    private async Task<Dictionary<string, decimal>> BuildContextAsync(OrderEntity order, CancellationToken ct)
    {
        var context = new Dictionary<string, decimal>(StringComparer.OrdinalIgnoreCase)
        {
            ["Quantity"] = order.Quantity
