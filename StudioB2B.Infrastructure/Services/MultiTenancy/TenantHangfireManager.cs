...
public sealed class TenantHangfireManager : IHostedService, IDisposable
{
    ...
    public async Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _logger.LogInformation("TenantHangfireManager: {Count} tenant server(s) started.", _tenants.Count);
    }
    ...
    public Task AddTenant(Guid tenantId, string connectionString, CancellationToken ct = default)
    {
        ...
        {
            _logger.LogDebug("TenantHangfireManager: server for tenant {TenantId} already registered, skipping.",
                             tenantId);
            return Task.CompletedTask;
        }

        CreateAndRegisterServer(tenantId, connectionString, subdomain: tenantId.ToString("N"));
        _logger.LogInformation("TenantHangfireManager: server for new tenant {TenantId} registered.",
                               tenantId);
        return Task.CompletedTask;
    }
    ...
    public IBackgroundJobClient GetClient(Guid tenantId)
    {
        ...
        throw new InvalidOperationException($"Hangfire client for tenant {tenantId} is not registered. " +
    ...
    public RecurringJobManager GetRecurringManager(Guid tenantId)
    {
        ...
        throw new InvalidOperationException($"Hangfire recurring manager for tenant {tenantId} is not registered.");
    }
    ...
    private void CreateAndRegisterServer(Guid tenantId, string connectionString, string subdomain)
    {
        try
        {
            ...
            {
                ctx.Dispose();
                _logger.LogDebug("TenantHangfireManager: race condition resolved for tenant {TenantId}.",
                                 tenantId);
            }
            else
            {
        ...
        {
            _logger.LogError(ex, "TenantHangfireManager: failed to start server for tenant {TenantId} ({Subdomain}).",
                tenantId, subdomain);
        }
    }
    ...
    private async Task CleanupStaleRecurringJobsAsync(Guid tenantId, string connectionString, CancellationToken ct)
    {
        try
        {
            ...
            if (deleted > 0)
                _logger.LogInformation("TenantHangfireManager: deleted {Count} stale job(s) with old signatures for tenant {TenantId}.",
                                       deleted, tenantId);
            ...
            if (jobIds.Count > 0)
            {
                ...
                _logger.LogInformation("TenantHangfireManager: removed {Count} stale recurring job(s) for tenant {TenantId}.",
                                       jobIds.Count, tenantId);
            }
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "TenantHangfireManager: failed to cleanup stale jobs for tenant {TenantId}.", tenantId);
        }
    }
    ...
    private async Task RestoreSchedulesAsync(Guid tenantId, string connectionString, CancellationToken ct = default)
    {
        try
        {
            ...
            {
                var job = Hangfire.Common.Job.FromExpression<OrderSyncJob>(j => j.ExecuteScheduledAsync(tenantId, connectionString, schedule.Id, ct));
                ctx.RecurringJobManager.AddOrUpdate(schedule.HangfireRecurringJobId!, job, schedule.CronExpression, new RecurringJobOptions());
            }

            _logger.LogInformation("TenantHangfireManager: restored {Count} schedule(s) for tenant {TenantId}.",
                                   schedules.Count, tenantId);
        }
        catch (Exception ex)
        {
            _logger.LogWarning(ex, "TenantHangfireManager: failed to restore schedules for tenant {TenantId}.", tenantId);
        }
    }
    ...
    public void Dispose()
    {
        foreach (var ctx in _tenants.Values)
        {
            ...
            {
                _logger.LogWarning(ex, "TenantHangfireManager: error disposing tenant Hangfire context.");
            }
        }
